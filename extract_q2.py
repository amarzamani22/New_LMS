...
# -------------------------------------------------
# Q2B – Islamic ops (Jun/Dec only), include sheet only if this FI has any non-zero
# -------------------------------------------------
def extract_q2b(ws, entity: str, yr: int, qlabel: str) -> pd.DataFrame:
   ...
   for wc, r in Q2B_ROWS_A.items():
       ...
       if val: nonzero_seen = True
       out.append({
           "entity_name": entity, "year": yr, "quarter": qlabel,
           "question": "Q2B",
           "subquestion": "A. Salary & Wages", "worker_category": wc, month: val
       })
   # Subsection B
   for wc, r in Q2B_ROWS_B.items():
       ...
       out.append({
           "entity_name": entity, "year": yr, "quarter": qlabel,
           "question": "Q2B",
           "subquestion": "B. Commissions Paid At Least Once a Month (RM)",
           "worker_category": wc, month: val
       })
   df = pd.DataFrame(out)
   # If everything is zero for this FI, suppress (return empty → staging won’t include it)
   if not nonzero_seen:
       return pd.DataFrame()
   return df
...
def main() -> int:
    ...
    def _sort(df: pd.DataFrame) -> pd.DataFrame:
        ...
        sort_cols = [c for c in ["entity_name","year","quarter","subquestion","worker_category","job_function"] if c in df.columns]
